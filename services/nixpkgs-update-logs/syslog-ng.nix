  ...
  options = {
    ...
    services.syslog-ng = {
      ...
      extraParams = mkOption {
        type = types.listOf types.str;
        default = [];
        example = [ "--startup-debug" "--verbose" "--debug" "--trace" "--stderr" ];
        description = ''
          Additional parameters passed to <command>syslog-ng</command>.
        '';
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    systemd.services.syslog-ng = {
      ...
      serviceConfig = {
        ...
        StandardOutput = "null";
        Restart = "on-failure";
        ExecStart = "${cfg.package}/sbin/syslog-ng ${concatStringsSep " " (syslogngOptions ++ cfg.extraParams)}";
